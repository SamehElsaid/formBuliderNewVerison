  ...
  const Select = useMemo(() => {
    return {
      Renderer: ({ data, onChange }) => {
        ...
        return (
          <>
            <ViewInputInFormEngine data={data}  advancedEdit={advancedEdit} locale={locale} defaultValue={defaultValue} onChange={onChange} type='select'/>
          </>
        )
      },
      id: 'select',
      ...
      controls: {
        type: 'custom',
        Component: ({ data, onChange }) => <CheckboxControl data={data} onChange={onChange} locale={locale} type='select' />
      }
    }
  }, [locale, advancedEdit])
